...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        # ========== collate_fn ==========
        use_offline_rollout = self.config.data.get("use_offline_rollout", False)

        from verl.utils.dataset.rl_dataset import collate_fn as default_collate_fn
        train_collate_fn = default_collate_fn
        val_collate_fn = default_collate_fn

        if use_offline_rollout:
            from verl.utils.dataset.offline_rollout_dataset import offline_rollout_collate_fn
            train_collate_fn = offline_rollout_collate_fn
            print("Using offline rollout collate_fn for training to preserve GRPO grouping")
            if getattr(self.val_dataset, "__class__", type(self.val_dataset)).__name__ == "OfflineRolloutDataset":
                val_collate_fn = offline_rollout_collate_fn
            else:
                val_collate_fn = default_collate_fn
        ...
        if val_batch_size is None or (isinstance(val_batch_size, int) and val_batch_size <= 0):
            val_batch_size = max(1, len(self.val_dataset))
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if not use_offline_rollout or "uid" not in batch.non_tensor_batch:
                ...
                # Offline/online rollout share the same outer timing scope so that
                # timing_raw["step"] is always populated.
                ...
                with marked_timer("step", timing_raw):
                    if use_offline_rollout:
                        ...
                        with marked_timer("gen", timing_raw, color="red"):
                            ...
                            if response_mask is not None:
                                response_attention_mask = response_mask.to(dtype=prompt_attention_mask.dtype)
                            else:
                            ...
                            response_position_ids = prompt_position_ids[:, -1:] + delta_position
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        if use_offline_rollout:
                            if "token_level_scores" in batch.batch:
                                print(f"[Step {self.global_steps}] 使用离线数据预计算的reward")
                                reward_tensor = batch.batch["token_level_scores"]
                                reward_extra_infos_dict = {}
                            else:
                                print(f"[Step {self.global_steps}] 重新计算reward")
                                if self.use_rm and "rm_scores" not in batch.batch.keys():
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        if "rollout_log_probs" in batch.batch.keys():
                            ...
                            if use_offline_rollout:
                                small_log_prob = batch.batch["rollout_log_probs"]
                                large_log_prob = batch.batch["old_log_probs"]
                                
                                prob_diff = (large_log_prob - small_log_prob).abs()
                                response_mask = batch.batch["response_mask"]
                                masked_diff = (prob_diff * response_mask).sum() / response_mask.sum()
                                
                                metrics.update({
                                    "mismatch/small_large_log_prob_diff_mean": masked_diff.item(),
                                    "mismatch/small_large_log_prob_diff_max": prob_diff.max().item(),
                                })
                    # ==================================================
